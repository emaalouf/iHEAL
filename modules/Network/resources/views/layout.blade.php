                ...
                @if ($version = cache()->store('file')->get(cache_prefix('check_cms_update')))
                    @if ($version != 1)
                        <div class="alert alert-warning w-50 ml-3">
                            <a href="https://juzaweb.com/documentation/changelog">iHealCMS {{ $version }}</a> {{ __('is available!') }} <a href="{{ route('admin.update') }}">{{ __('Please update now') }}</a>.
                        </div>
                    @endif
                @endif
            ...
            <div class="juzaweb__layout__footer">
                <div class="juzaweb__footer">
                    <div class="juzaweb__footer__inner">
                        ...
                        <br />
                        <p class="mb-0">
                            Copyright © {{ date('Y') }} {{ get_config('title') }} - iHeal CMS
                        </p>
                    </div>
                </div>
            </div>
